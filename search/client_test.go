...
func TestSearchRoundTrip(t *testing.T) {
			...
			return []search.Result{
				{
					...
					PublishedAt: rootpuller.Ptr("2026-08-01"),
				},
	...
	resp, err := c.Search(t.Context(), &search.Request{
		...
		MaxResults: rootpuller.Ptr(5),
		Country:    rootpuller.Ptr("DE"),
		Language:   rootpuller.Ptr("de"),
...
func TestSearchOptionalFieldsRoundTrip(t *testing.T) {
	...
	if msg.Type == nil || *msg.Type != searchpb.SearchType_SEARCH_TYPE_IMAGES {
		t.Errorf("Type = %v, want SEARCH_TYPE_IMAGES", msg.Type)
	}
	if msg.MaxResults == nil || *msg.MaxResults != 7 {
		t.Errorf("MaxResults = %v, want 7", msg.MaxResults)
	}
	if msg.Country == nil || *msg.Country != "DE" {
		t.Errorf("Country = %v, want DE", msg.Country)
	}
	if msg.Language == nil || *msg.Language != "de" {
		t.Errorf("Language = %v, want de", msg.Language)
	}
	if msg.Freshness == nil || *msg.Freshness != searchpb.Freshness_FRESHNESS_MONTH {
