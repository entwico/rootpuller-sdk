...
func (failingTokenSource) Token() (*oauth2.Token, error) {
	return nil, errors.New("keycloak unreachable")
}
...
func TestServerErrorWithRetryInfo(t *testing.T) {
	srv := rootpullertest.NewServer(t, &rootpullertest.Chunker{
		ChunkFunc: func(string, []string) ([][]common.TextChunk, error) {
			cerr := connect.NewError(connect.CodeResourceExhausted, errors.New("deployment saturated"))
...
func TestAllMethodsDispatch(t *testing.T) {
	...
	calls := []struct {
		...
		{"ChunkSemantic", func() error { _, e := c.Chunker().ChunkSemantic(ctx, &chunker.SemanticRequest{Texts: texts}); return e }},
		{"ChunkToken", func() error { _, e := c.Chunker().ChunkToken(ctx, &chunker.TokenRequest{Texts: texts}); return e }},
		{"ChunkSentence", func() error { _, e := c.Chunker().ChunkSentence(ctx, &chunker.SentenceRequest{Texts: texts}); return e }},
		{"ChunkCode", func() error { _, e := c.Chunker().ChunkCode(ctx, &chunker.CodeRequest{Texts: texts}); return e }},
		{"ChunkRecursive", func() error {
			_, e := c.Chunker().ChunkRecursive(ctx, &chunker.RecursiveRequest{Texts: texts})
			return e
		}},
		{"ChunkLate", func() error { _, e := c.Chunker().ChunkLate(ctx, &chunker.LateRequest{Texts: texts}); return e }},
		{"ChunkNeural", func() error { _, e := c.Chunker().ChunkNeural(ctx, &chunker.NeuralRequest{Texts: texts}); return e }},
		{"ChunkSlumber", func() error { _, e := c.Chunker().ChunkSlumber(ctx, &chunker.SlumberRequest{Texts: texts}); return e }},
	}
